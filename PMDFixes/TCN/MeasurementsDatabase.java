...
public class MeasurementsDatabase {
    ...
    private static volatile MeasurementsDatabase instance;
    ...
    public boolean insertMeasurement(Measurement measurement) {
        ...
        try {
            db.beginTransaction();
            StringBuilder resultSb = new StringBuilder(77);
            ...
            for (Cell cell : measurement.getCells()) {
                ...
                if (result) {
                    ...
                    boolean localResult = false;
                    if (rowId != -1) {
                        cell.setCellSignalId((int) rowId);
                        localResult = true;
                    }
                    result = localResult;
    ...
    public Measurement getFirstMeasurement() {
        ...
        List<Measurement> measurements = getMeasurements(CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + " = (SELECT tm." + MeasurementsTable.COLUMN_ROW_ID + " FROM " + NotUploadedMeasurementsView.VIEW_NAME + " tm ORDER BY tm." + MeasurementsTable.COLUMN_MEASURED_AT + " ASC, tm." + MeasurementsTable.COLUMN_ROW_ID + " ASC LIMIT 0,1)", null, null, null, NotUploadedMeasurementsView.VIEW_NAME + "." + MeasurementsTable.COLUMN_MEASURED_AT + " ASC, " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_NEIGHBORING + " ASC, " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_ROW_ID + " ASC", null, false);
    ...
    public Measurement getLastMeasurement() {
        ...
        List<Measurement> measurements = getMeasurements(CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + " = (SELECT tm." + MeasurementsTable.COLUMN_ROW_ID + " FROM " + NotUploadedMeasurementsView.VIEW_NAME + " tm ORDER BY tm." + MeasurementsTable.COLUMN_MEASURED_AT + " DESC, tm." + MeasurementsTable.COLUMN_ROW_ID + " DESC LIMIT 0,1)", null, null, null, NotUploadedMeasurementsView.VIEW_NAME + "." + MeasurementsTable.COLUMN_MEASURED_AT + " DESC, " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_NEIGHBORING + " ASC, " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_ROW_ID + " DESC", null, false);
        if (!measurements.isEmpty()) {
    ...
    public Statistics getMeasurementsStatistics() {
        ...
        String uploadToOcidAndMlsQuery = "SELECT SUM(CASE WHEN m." + MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT + " IS NULL THEN 1 ELSE 0 END) AS " + uploadToOcid + ", " + "SUM(CASE WHEN m." + MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT + " IS NULL THEN 1 ELSE 0 END) AS " + uploadToMls + " FROM " + CellSignalsTable.TABLE_NAME + " cs INNER JOIN " + MeasurementsTable.TABLE_NAME + " m ON cs." + CellSignalsTable.COLUMN_MEASUREMENT_ID + " = m." + MeasurementsTable.COLUMN_ROW_ID;
        String query = "SELECT * FROM ((" + globalStatsQuery + ") " + "JOIN (" + localMeasurementsAndCellsQuery + ") " + "JOIN (" + localDiscoveredCellsQuery + ") " + "JOIN (" + localSinceQuery + ") " + "JOIN (" + todayMeasurementsAndCellsQuery + ") " + "JOIN (" + todayDiscoveredCellsQuery + ") " + "JOIN (" + uploadToOcidAndMlsQuery + "))";
    ...
    public AnalyticsStatistics getAnalyticsStatistics() {
        ...
        if (cursor.moveToNext()) {
            ...
            int diff = (int) DateUtils.getTimeDiff(maxDate, minDate);
            stats.setDays(diff);
        }
        cursor.close();
    ...
    public Boundaries getLocationBounds() {
        ...
        SQLiteDatabase db = helper.getReadableDatabase();
        String query = "SELECT MIN(" + MeasurementsTable.COLUMN_LATITUDE + ") AS MIN_LAT, MIN(" + MeasurementsTable.COLUMN_LONGITUDE + ") AS MIN_LON, MAX(" + MeasurementsTable.COLUMN_LATITUDE + ") AS MAX_LAT, MAX(" + MeasurementsTable.COLUMN_LONGITUDE + ") AS MAX_LON" + " FROM " + NotUploadedMeasurementsView.VIEW_NAME;
        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToNext()) {
    ...
    public List<Measurement> getMeasurementsPartIncludingPartiallyUploaded(int offset, int limit) {
        ...
        return getMeasurements(CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + " IN(SELECT tm." + MeasurementsTable.COLUMN_ROW_ID + " FROM " + MeasurementsTable.TABLE_NAME + " tm ORDER BY tm." + MeasurementsTable.COLUMN_MEASURED_AT + " ASC, tm." + MeasurementsTable.COLUMN_ROW_ID + " ASC LIMIT " + offset + ", " + limit + ")", null, null, null, null, null, true);
    }
    ...
    public List<Measurement> getMeasurementsPart(int offset, int limit) {
        ...
        return getMeasurements(CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + " IN(SELECT tm." + MeasurementsTable.COLUMN_ROW_ID + " FROM " + NotUploadedMeasurementsView.VIEW_NAME + " tm ORDER BY tm." + MeasurementsTable.COLUMN_MEASURED_AT + " ASC, tm." + MeasurementsTable.COLUMN_ROW_ID + " ASC LIMIT " + offset + ", " + limit + ")", null, null, null, null, null, false);
    }
    ...
    private List<Measurement> getMeasurements(String selection, String[] selectionArgs, String groupBy, String having, String sortOrder, String limit, boolean includePartiallyUploaded) {
        ...
        SQLiteDatabase db = helper.getReadableDatabase();
        SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
        queryBuilder.setTables(MEASUREMENTS_TABLE + " INNER JOIN " + CellSignalsTable.TABLE_NAME + " ON (" + MEASUREMENTS_TABLE + "." + MeasurementsTable.COLUMN_ROW_ID + " = " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + ")" + " INNER JOIN " + CellsTable.TABLE_NAME + " ON (" + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_CELL_ID + " = " + CellsTable.TABLE_NAME + "." + CellsTable.COLUMN_ROW_ID + ")");
        String[] returnedColumns = { MEASUREMENTS_TABLE + "." + MeasurementsTable.COLUMN_ROW_ID + " AS " + MEASUREMENT_ROW_ID, CellsTable.TABLE_NAME + "." + CellsTable.COLUMN_ROW_ID + " AS " + CELL_ROW_ID, CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_ROW_ID + " AS " + CELL_SIGNAL_ROW_ID, CellSignalsTable.COLUMN_PSC, CellSignalsTable.COLUMN_NEIGHBORING, CellSignalsTable.COLUMN_TA, CellSignalsTable.COLUMN_ASU, CellSignalsTable.COLUMN_DBM, CellSignalsTable.COLUMN_RSRP, CellSignalsTable.COLUMN_RSRQ, CellSignalsTable.COLUMN_RSSI, CellSignalsTable.COLUMN_RSSNR, CellSignalsTable.COLUMN_CQI, CellSignalsTable.COLUMN_RSCP, CellSignalsTable.COLUMN_CSI_RSRP, CellSignalsTable.COLUMN_CSI_RSRQ, CellSignalsTable.COLUMN_CSI_SINR, CellSignalsTable.COLUMN_SS_RSRP, CellSignalsTable.COLUMN_SS_RSRQ, CellSignalsTable.COLUMN_SS_SINR, CellSignalsTable.COLUMN_CDMA_DBM, CellSignalsTable.COLUMN_CDMA_ECIO, CellSignalsTable.COLUMN_EVDO_DBM, CellSignalsTable.COLUMN_EVDO_ECIO, CellSignalsTable.COLUMN_EVDO_SNR, CellSignalsTable.COLUMN_EC_NO, CellSignalsTable.COLUMN_ARFCN, MeasurementsTable.COLUMN_MEASURED_AT, MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT, MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT, MeasurementsTable.COLUMN_LATITUDE, MeasurementsTable.COLUMN_LONGITUDE, MeasurementsTable.COLUMN_GPS_ACCURACY, MeasurementsTable.COLUMN_GPS_SPEED, MeasurementsTable.COLUMN_GPS_BEARING, MeasurementsTable.COLUMN_GPS_ALTITUDE, CellsTable.COLUMN_CID, CellsTable.COLUMN_LAC, CellsTable.COLUMN_MNC, CellsTable.COLUMN_MCC, CellsTable.COLUMN_NET_TYPE, CellsTable.COLUMN_DISCOVERED_AT };
    ...
    public List<MapMeasurement> getMeasurementsInArea(Boundaries boundaries) {
        ...
        SQLiteDatabase db = helper.getReadableDatabase();
        SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
        queryBuilder.setTables(NotUploadedMeasurementsView.VIEW_NAME + " INNER JOIN " + CellSignalsTable.TABLE_NAME + " ON (" + NotUploadedMeasurementsView.VIEW_NAME + "." + MeasurementsTable.COLUMN_ROW_ID + " = " + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_MEASUREMENT_ID + ")" + " INNER JOIN " + CellsTable.TABLE_NAME + " ON (" + CellSignalsTable.TABLE_NAME + "." + CellSignalsTable.COLUMN_CELL_ID + " = " + CellsTable.TABLE_NAME + "." + CellsTable.COLUMN_ROW_ID + ")");
        String[] returnedColumns = { NotUploadedMeasurementsView.VIEW_NAME + "." + MeasurementsTable.COLUMN_ROW_ID + " AS " + MEASUREMENT_ROW_ID, CellSignalsTable.COLUMN_NEIGHBORING, MeasurementsTable.COLUMN_LATITUDE, MeasurementsTable.COLUMN_LONGITUDE, MeasurementsTable.COLUMN_MEASURED_AT, CellsTable.COLUMN_CID, CellsTable.COLUMN_LAC, CellsTable.COLUMN_MNC, CellsTable.COLUMN_MCC, CellsTable.COLUMN_NET_TYPE, CellsTable.COLUMN_DISCOVERED_AT };
        String selection = MeasurementsTable.COLUMN_LATITUDE + " > ?" + " AND " + MeasurementsTable.COLUMN_LATITUDE + " < ?" + " AND " + MeasurementsTable.COLUMN_LONGITUDE + " > ?" + " AND " + MeasurementsTable.COLUMN_LONGITUDE + " < ?";
        String[] selectionArgs = new String[] { String.valueOf(boundaries.getMinLat()), String.valueOf(boundaries.getMaxLat()), String.valueOf(boundaries.getMinLon()), String.valueOf(boundaries.getMaxLon()) };
    ...
    public int markAsUploaded(int[] measurementIds, Long uploadedToOcidAt, Long uploadedToMlsAt) {
        ...
        try {
            ...
            for (int i = 0; i < numOfMoves; i++) {
                ...
                StringBuilder whereClauseBuilder = new StringBuilder(MeasurementsTable.COLUMN_ROW_ID);
    ...
    public int clearOlderUploadedPartiallyAndUploadedFully() {
        ...
        try {
            ...
            deletedCellSignals = db.delete(CellSignalsTable.TABLE_NAME, CellSignalsTable.COLUMN_MEASUREMENT_ID + " IN (SELECT DISTINCT " + MeasurementsTable.COLUMN_ROW_ID + " FROM " + MeasurementsTable.TABLE_NAME + " WHERE " + MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT + " < ? OR " + MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT + " < ?" + " OR (" + MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT + " IS NOT NULL AND " + MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT + " IS NOT NULL))", new String[] { String.valueOf(minTimeToKeep), String.valueOf(minTimeToKeep) });
    ...
    public String quickDump() {
        ...
        SQLiteDatabase db = helper.getReadableDatabase();
        {
            sb.append("--CELLS TABLE ").append(CellsTable.COLUMN_ROW_ID).append(',').append(CellsTable.COLUMN_DISCOVERED_AT).append("\r\n");
            String query = "SELECT " + CellsTable.COLUMN_ROW_ID + "," + CellsTable.COLUMN_DISCOVERED_AT + " FROM " + CellsTable.TABLE_NAME;
            Cursor cursor = db.rawQuery(query, null);
            while (cursor.moveToNext()) {
                int rowId = cursor.getInt(0);
                long discoveredAt = cursor.getLong(1);
                sb.append(rowId).append(',').append(discoveredAt).append("\r\n");
            }
            cursor.close();
        }
        {
            sb.append("--MEASUREMENTS TABLE ").append(MeasurementsTable.COLUMN_ROW_ID).append(',').append(MeasurementsTable.COLUMN_MEASURED_AT).append(',').append(MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT).append(',').append(MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT).append("\r\n");
            String query = "SELECT " + MeasurementsTable.COLUMN_ROW_ID + "," + MeasurementsTable.COLUMN_MEASURED_AT + "," + MeasurementsTable.COLUMN_UPLOADED_TO_OCID_AT + "," + MeasurementsTable.COLUMN_UPLOADED_TO_MLS_AT + " FROM " + MeasurementsTable.TABLE_NAME;
            Cursor cursor = db.rawQuery(query, null);
            while (cursor.moveToNext()) {
                int rowId = cursor.getInt(0);
                ...
                sb.append(rowId).append(",").append(measuredAt).append(",").append(uploadedToOcidAt).append(",").append(uploadedToMlsAt).append("\r\n");
            }
            cursor.close();
        }
        {
            sb.append("--CELL_SIGNALS TABLE ").append(CellSignalsTable.COLUMN_ROW_ID).append(',').append(CellSignalsTable.COLUMN_CELL_ID).append(',').append(CellSignalsTable.COLUMN_MEASUREMENT_ID).append(',').append(CellSignalsTable.COLUMN_NEIGHBORING).append("\r\n");
            String query = "SELECT " + CellSignalsTable.COLUMN_ROW_ID + "," + CellSignalsTable.COLUMN_CELL_ID + "," + CellSignalsTable.COLUMN_MEASUREMENT_ID + "," + CellSignalsTable.COLUMN_NEIGHBORING + " FROM " + CellSignalsTable.TABLE_NAME;
            Cursor cursor = db.rawQuery(query, null);
            while (cursor.moveToNext()) {
                int rowId = cursor.getInt(0);
                ...
                sb.append(rowId).append(',').append(cellId).append(',').append(measurementId).append(',').append(isNeighboring).append("\r\n");
            }
            cursor.close();
        }
        {
            sb.append("--STATS TABLE ").append(StatsTable.COLUMN_ROW_ID).append(',').append(StatsTable.COLUMN_TOTAL_MEASUREMENTS).append(',').append(StatsTable.COLUMN_TOTAL_DISCOVERED_CELLS).append(',').append(StatsTable.COLUMN_TOTAL_SINCE).append("\r\n");
            String query = "SELECT " + StatsTable.COLUMN_ROW_ID + "," + StatsTable.COLUMN_TOTAL_MEASUREMENTS + "," + StatsTable.COLUMN_TOTAL_DISCOVERED_CELLS + "," + StatsTable.COLUMN_TOTAL_SINCE + " FROM " + StatsTable.TABLE_NAME;
            Cursor cursor = db.rawQuery(query, null);
            while (cursor.moveToNext()) {
                int rowId = cursor.getInt(0);
                ...
                sb.append(rowId).append(',').append(totalMeasurements).append(',').append(discoveredCells).append(',').append(totalSince).append("\r\n");
            }
            cursor.close();
        }
        return sb.toString();
    ...
    public boolean hasValidSchema() {
        ...
        SQLiteDatabase db = helper.getReadableDatabase();
        int count = -1;
        String query = "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN ('" + MeasurementsTable.TABLE_NAME + "'" + ", '" + CellsTable.TABLE_NAME + "', '" + CellSignalsTable.TABLE_NAME + "', '" + StatsTable.TABLE_NAME + "')";
        Cursor cursor = db.rawQuery(query, null);
        if (cursor.moveToNext()) {
