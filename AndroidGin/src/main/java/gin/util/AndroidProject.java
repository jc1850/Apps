...
public class AndroidProject extends Project {
    ...
    public AndroidProject(AndroidConfig config){
        ...
        //monitor.setPid(ProcessHandle.current().pid());
        //monitor.start();
    }
    ...
    protected void copyUnitTestsToInstrumentation(){
        if (!this.unitTestsCopied){
            for (File testDir: unitTestSourceDirs){
                for(File test: testDir.listFiles()) {
                    String newPath = StringUtils.replaceOnce(test.getAbsolutePath(),SepWrap("test"), SepWrap("androidTest"));

                    newPath = newPath.replace(".java", "MovedByGin.java");
                    File newFile = new File(newPath);
                    try {
                        FileUtils.copyFile(test, newFile);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    instrumentedTestDirs.add(newFile.getParentFile());
                }

            }
            this.unitTestsCopied = true;
        }
    }

    public void removeCopiedTests(){
        for (File dir : instrumentedTestDirs){
            for (File child : dir.listFiles()){
                if (child.getName().contains("MovedByGin")){
                    child.delete();
                }
            }
        }
...
//                memorySampler.start();
                while ((line = stdin.readLine()) != null) {
                        ...
                        if (line.contains("Gin Memory: ")) {
                            String[] tokens = line.split(" ");
                            memory += Long.parseLong(tokens[6]) / 1000;
                        }

                }
                p.waitFor();
                p.destroy();
                Long netUsed = getNetworkUsed() - netBefore;
...
//                result.setExecutionMemory(memory);
    ...
    public void replaceTestsWithEspresso(){
        instrumentedTests = new ArrayList<>();
        unitTests = new ArrayList<>();
        //get instrumented tests
        for (File testDir : instrumentedTestDirs) {
            for (File testFile : testDir.listFiles()) {
                if (testFile.isFile()) {
                    ...
                    try {
                        ...
                        System.err.println("Failed to read file: " + testFile);
                        e.printStackTrace();
                    }
